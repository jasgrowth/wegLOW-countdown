...
// Try to register brand fonts if present; fall back silently if not
function registerFonts() {
  ...
  for (const c of candidates) {
    try {
      const p = path.join(process.cwd(), 'public', 'fonts', c.file);
...
module.exports = async (req, res) => {
  try {
    ...
    // numbers
    ctx.font = `800 ${numSize}px "Montserrat ExtraBold", "Montserrat Bold", Montserrat, Arial, Helvetica, sans-serif`;
    const vals = [dd, hh, mm, ss];
    const labels = ['DAYS', 'HOURS', 'MINUTES', 'SECONDS'];
    centers.forEach((cx, i) => {
      ctx.fillText(vals[i], cx, Math.round(height * 0.48));
    });

    // colons
    ctx.font = `800 ${colonSize}px "Montserrat ExtraBold", "Montserrat Bold", Montserrat, Arial, Helvetica, sans-serif`;
    colonX.forEach((cx) => ctx.fillText(':', cx, Math.round(height * 0.45)));

    // labels
    ctx.font = `600 ${labelSize}px "Montserrat SemiBold", Montserrat, Arial, Helvetica, sans-serif`;
    centers.forEach((cx, i) => {
      ctx.fillText(labels[i], cx, Math.round(height * 0.82));
    });
